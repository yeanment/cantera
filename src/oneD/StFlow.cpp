                               ...
                               double rdt, size_t jmin, size_t jmax)
{
    ...
    if (jmax == m_points - 1) { // right boundary
        rsd[index(c_offset_E, jmax)] = x[index(c_offset_E, jmax)];    
        if (m_usesLambda && m_arcLengthCont) {
            rsd[index(c_offset_E, jmax)] = x[index(c_offset_E, jmax)] - x[index(c_offset_E, jmax-1)];
            if (grid(jmax) == m_zTMax) {
                // Modified internal boundary
                double dTmax = (T(x, jmax) - m_tMaxPrev)/m_deltaTmaxRef; // Ref dT = 10
                double dlnmdot = (x[index(c_offset_E, jmax)] - m_lnmdotPrev)/m_deltaLnmdotRef;
                rsd[index(c_offset_E, jmax)] = dTmax*m_dTmaxds + dlnmdot*m_dlnmdotds - m_ds;
            }
            diag[index(c_offset_E, jmax)] = 0;
        } 
    }
